...
load(fullfile(folder, '10mm_101_short.mat'));
...
title(sprintf('Abs - Voxel (%d, %d): T1=%.0f, T2=%.0f', x, y, T1, T2));
...
title(sprintf('Real - Voxel (%d, %d): T1=%.0f, T2=%.0f', x, y, T1, T2));
...
title(sprintf('Imag - Voxel (%d, %d): T1=%.0f, T2=%.0f', x, y, T1, T2));
...
plot(abs(dict_sig_aligned), 'r');
legend('Measured signal (aligned)', 'Matched dictionary signal (scaled)');
xlabel('Timepoint'); ylabel('|Signal|');
title(sprintf('Abs - Voxel (%d, %d): T1=%.0f, T2=%.0f', x, y, T1, T2));

figure;
plot(real(meas_sig), 'k--'); hold on;
plot(real(dict_sig_aligned), 'r');
legend('Measured signal (aligned)', 'Matched dictionary signal (scaled)');
