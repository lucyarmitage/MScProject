...
out_folder = joinpath("progress", "progress_prostate_20mm_101_transverse")
out_file   = "blochdict_prostate_20mm_101_transverse.mat"
...
seq = read_seq("mpf_001_prostate_short.seq")

z_pos = collect(range(-10e-3, 10e-3, length=101))
...
for (i, (T1, T2)) in enumerate(sampled_pairs_s)
    ...
    obj = Phantom{Float64}(
        x = zeros(length(z_pos)),
        y = zeros(length(z_pos)),
        z = z_pos,
        T1 = fill(T1, length(z_pos)),
        T2 = fill(T2, length(z_pos))
    )
